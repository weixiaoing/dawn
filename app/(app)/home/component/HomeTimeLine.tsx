...
import { getBlogList, getTalksByPage } from "@/utils";
...
export default function HomeTimeLine() {
  ...
  return (
    <div className="flex flex-col xl:flex-row w-full justify-center gap-4 [&>section]:flex-1">
      ...
      <section className="px-4">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-2xl ml-2">最近动态</h1>
          <main className="h-[200px] mt-10">
            {
              <TimeLine>
                {(!talks || talks.length == 0) && "暂无动态"}
                {talks &&
                  talks.map((event) => {
                    return (
                      <li
